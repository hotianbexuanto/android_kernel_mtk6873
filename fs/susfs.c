...
int susfs_add_sus_path(struct st_susfs_sus_path* __user user_info) {
	...
	if (strstr(resolved_pathname, android_data_path)) {
		...
		SUSFS_LOGE("Path '%s' is not under android_data_path '%s' or sdcard_path '%s'\n",
				   resolved_pathname, android_data_path, sdcard_path);
		err = -EINVAL;
		goto out_kfree_tmp_buf;
	}
...
static inline bool is_i_uid_not_allowed(uid_t i_uid) {
	return is_i_uid_in_android_data_not_allowed(i_uid) || is_i_uid_in_sdcard_not_allowed(i_uid);
}
...
bool susfs_is_sus_android_data_d_name_found(const char *d_name) {
	...
	list_for_each_entry_safe(cursor, temp, &LH_SUS_PATH_ANDROID_DATA, list) {
		if (d_name_len == cursor->path_len && cursor->info.i_uid != 0 && is_i_uid_not_allowed(current_uid().val)) {
			if (unlikely(!strcmp(cursor->info.target_pathname, d_name))) {
				return true;
			}
		}
	}
...
int susfs_set_media_provider_uid(unsigned int uid) {
	SUSFS_LOGI("Media provider uid: %u\n", uid);
	return 0;
}
